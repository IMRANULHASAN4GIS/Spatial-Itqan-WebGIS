...
/* ---- map + basemaps ---- */
/* Esri-style startup: first use opens neutrally; later visits restore the last view.
   Location remains opt-in through Locate Me and is never requested at startup. */
var HOME={center:[20,0],zoom:2};
var MAP_VIEW_KEY='SpatialItqanLastMapViewV110';
var MAP_MAX_ZOOM=28;
function validSavedMapView(value){
  return value&&Number.isFinite(+value.lat)&&Number.isFinite(+value.lng)&&Number.isFinite(+value.zoom)&&
    +value.lat>=-90&&+value.lat<=90&&+value.lng>=-180&&+value.lng<=180&&
    +value.zoom>=0&&+value.zoom<=MAP_MAX_ZOOM;
}
function loadSavedMapView(){
  try{var saved=JSON.parse(localStorage.getItem(MAP_VIEW_KEY)||'null');return validSavedMapView(saved)?saved:null;}catch(e){return null;}
}
function clearSavedMapView(){try{localStorage.removeItem(MAP_VIEW_KEY);}catch(e){}}
var initialMapView=loadSavedMapView();
var map=L.map('map',{
  zoomControl:false,
  center:initialMapView?[+initialMapView.lat,+initialMapView.lng]:HOME.center,
  zoom:initialMapView?+initialMapView.zoom:HOME.zoom,
  minZoom:1,
...
var saveMapViewTimer=null;
function saveCurrentMapView(){
  clearTimeout(saveMapViewTimer);
  saveMapViewTimer=setTimeout(function(){
    try{
      var center=map.getCenter(),zoom=map.getZoom();
      localStorage.setItem(MAP_VIEW_KEY,JSON.stringify({lat:center.lat,lng:center.lng,zoom:zoom,savedAt:new Date().toISOString()}));
    }catch(e){}
  },180);
}
...
function sbaEvalExpr(expr,props,feature,rowIndex){
  ...
  try{
    if(!window.SpatialExpression)throw new Error('Restricted expression engine is unavailable');
    var context=Object.assign({},props||{},g,{ROW_NUMBER:(rowIndex||0)+1});
    return window.SpatialExpression.evaluate(raw,context,{UPPER:UPPER,LOWER:LOWER,TRIM:TRIM,CONCAT:CONCAT,COALESCE:COALESCE,ROUND:ROUND,ABS:Math.abs,MIN:Math.min,MAX:Math.max,SQRT:Math.sqrt,NOW:NOW});
...
function pdRestoreFromHash(){
  ...
  try{
    ...
    if(st.t){applyTheme(st.t); var ts=document.getElementById('themeSelect'); if(ts&&THEME_NAMES.indexOf(st.t)>=0)ts.value=st.t;}
...
function gpFieldCalc(){
  ...
  L0.geojson.features.forEach(function(f){
    ...
    try{
      ...
      if(!window.SpatialExpression)throw new Error('Restricted expression engine is unavailable');
      var val=window.SpatialExpression.evaluate(e,scope,{ABS:Math.abs,MIN:Math.min,MAX:Math.max,ROUND:Math.round,SQRT:Math.sqrt});
...
function autoSaveNow(){
  try{
    ...
    if(window.SpatialStorage)window.SpatialStorage.saveProject('autosave',obj).catch(function(){});
  }catch(e){}
}
...
var THEME_BASEMAP={
  'professional-navy':'voyager','survey-white':'lightgray','graphite-gis':'voyager','desert-survey':'topo',
  graphite:'lightgray', arctic:'lightgray', midnight:'voyager'
};
var THEME_NAMES=['professional-navy','survey-white','graphite-gis','desert-survey','graphite','arctic','midnight'];
...
function applyTheme(val){
  var themeName = THEME_NAMES.indexOf(val)>=0?val:'professional-navy';
  document.body.setAttribute('data-theme', themeName);
  try{localStorage.setItem('SpatialItqanThemeV110',themeName);}catch(e){}
  try{var meta=document.querySelector('meta[name="theme-color"]');if(meta)meta.setAttribute('content',getComputedStyle(document.body).getPropertyValue('--panel').trim()||'#142033');}catch(e){}
  applyThemeBasemap(themeName);
}
var themeSel=document.getElementById('themeSelect');
var savedTheme='professional-navy';try{savedTheme=localStorage.getItem('SpatialItqanThemeV110')||savedTheme;}catch(e){}
if(THEME_NAMES.indexOf(savedTheme)<0)savedTheme='professional-navy';
...
function runProCommand(){
  ...
  else if(q.indexOf('dark')>=0){themeSel.value='graphite-gis';applyTheme('graphite-gis');proLog('Graphite GIS theme applied.');}
  else if(q.indexOf('light')>=0||q.indexOf('gray')>=0||q.indexOf('grey')>=0){themeSel.value='survey-white';applyTheme('survey-white');proLog('Survey White theme applied.');}
...
function saveProjectBrowser(){var project=exportProjectObject();localStorage.setItem('SpatialViewUltimateProject',JSON.stringify(project));if(window.SpatialStorage)window.SpatialStorage.saveProject('default',project).then(function(){proLog('Project saved in durable browser storage.');}).catch(function(){proLog('Project saved in fallback browser storage.');});else proLog('Project saved in this browser.');}
function restoreProject(p){
  ...
  if(p.theme){try{applyTheme(p.theme);if(themeSel&&THEME_NAMES.indexOf(p.theme)>=0)themeSel.value=p.theme;}catch(e){}}
...
function restoreBrowserProject(){if(window.SpatialStorage){window.SpatialStorage.loadProject('default').then(function(project){if(project){restoreProject(project);return;}var fallback=localStorage.getItem('SpatialViewUltimateProject');if(fallback)restoreProject(JSON.parse(fallback));else proLog('No browser-saved project found.');}).catch(function(e){proLog('Could not restore browser project: '+e.message);});return;}var s=localStorage.getItem('SpatialViewUltimateProject');if(!s){proLog('No browser-saved project found.');return;}try{restoreProject(JSON.parse(s));}catch(e){proLog('Could not restore browser project: '+e.message);}}
...
document.getElementById('refreshBtn').onclick=function(){
  if(!confirm('Factory reset this Spatial Itqan session? This clears loaded layers, points, sketches, measurements and the saved map view.')) return;
  clearSavedMapView();
  ...
  toast('Session reset to the neutral world view');
};
...
(function(){
  ...
  function saveEditsToLocal(){
    try{
      ...
      if(window.SpatialStorage)window.SpatialStorage.saveProject('default',pack).catch(function(){});
      safeToast('Edits saved to durable browser storage');
  ...
  function cycleTheme(){
    ...
    var themes=['professional-navy','survey-white','graphite-gis','desert-survey','graphite','arctic','midnight'];
